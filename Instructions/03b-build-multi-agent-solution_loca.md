...
> **Note**: `Pass`, if you konw `Azure AI Foundry project endpoint`, you can skip this step and go to the next step `Create an AI Agent client app`
...
1. Enter the following comand to open the project for this lab `lab 03b`

    ```
    code .
    ```

1. Edit the configuration file that is provided:
...
1. After you've replaced the placeholders, use the **CTRL+S** command to save your changes.

### Create AI agents

Now you're ready to create the agents for your multi-agent solution! Let's get started!

1. Open the following file to edit the **agent_triage.py** file:

    ```
   open agent_triage.py
    ```
...
1. Use the **CTRL+S** command to save your changes to the code file. You can keep it open (in case you need to edit the code to fix any errors) or use the **CTRL+Q** command to close the code editor.
...
1. After you have signed in, enter the following command to run the application in a terminal:
...
> **Note**: If you are going to continue to the next lab, ignore the `Clean up` and move to the next lab
